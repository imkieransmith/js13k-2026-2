...
const layerColours = {
  ...
  ruins: '#fff3a6', bushes: '#469a45', collision: '#ff315a',
};
...
const inspectOrder = ['collision', 'water', 'stones', 'bushes', 'ruins', 'dirt', 'grass'];
const inspectLabels = { collision: 'Collision', water: 'Water', stones: 'Stones', bushes: 'Bushes', ruins: 'Floor', dirt: 'Dirt', grass: 'Grass' };
...
function updateInspector() {
  ...
  for (const kind of inspectOrder) {
    ...
    let state = 'Empty';
    if (count) {
      if (kind === 'collision') state = `${count}/16 blocking`;
    ...
    row.className = `editor__inspector-layer${count ? ' is-present' : ''}`;
...
function markPending(x, y, size, value) {
  pendingCells.set(`${size}:${x}:${y}`, { x, y, size, value, kind: selectedKind });
}

function paintCell(x, y, value, size = gridSize()) {
  const changed = paintGridCell(level, selectedKind, x, y, value, size);
  if (changed) markPending(x, y, size, value);
  return changed;
}
...
function flood(cell, value) {
  ...
  const target = gridAt(level, selectedKind, cell.x, cell.y, size);
  ...
  while (stack.length) {
    ...
    if (visited[index] || gridAt(level, selectedKind, x, y, size) !== target) continue;
    visited[index] = 1;
...
function finishGesture(commit = true) {
  if (panning) {
    ...
    if (selectedMode === 'rectangle' && commit && startCell && rectangleCell) {
      ...
      const changed = paintGridRect(level, selectedKind, startCell.x, startCell.y,
        rectangleCell.x, rectangleCell.y, value, size);
...
document.querySelector('.editor__panel').addEventListener('click', async event => {
  ...
  if (inspectAction === 'clear' && inspectorCell) {
    ...
    if (paintGridCell(level, button.dataset.inspectKind,
      inspectorCell.x, inspectorCell.y, 0, AUTHOR_TILE)) {
      pushUndo(before);
      updateInspector();
      queueBuild(`Cleared ${inspectLabels[button.dataset.inspectKind]}`);
    }
    return;
  }
...
window.addEventListener('keydown', event => {
  ...
  if ('1234567'.includes(event.key)) selectedKind = TERRAIN_KINDS[Number(event.key) - 1];
...
function drawPending() {
  for (const cell of pendingCells.values()) {
    context.globalAlpha = cell.value ? 0.34 : 0.5;
    context.fillStyle = cell.value ? layerColours[cell.kind] : '#ff315a';
...
function drawCursor() {
  ...
  context.fillStyle = selectedMode === 'inspect'
    ? '#62b8ed'
    : selectedMode === 'erase' || painting && eraseStroke ? '#ff315a' : layerColours[selectedKind];
